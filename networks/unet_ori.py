...
class ConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dropout_p, flag_3d=False):
        ...
        if not flag_3d:
            ...
            self.conv_conv = nn.Sequential(
                ...
                nn.LeakyReLU(),
                nn.Dropout(dropout_p),
                nn.Conv3d(out_channels, out_channels, kernel_size=(3, 3, 3), stride=(1, 1, 1), padding=1),
                # nn.BatchNorm3d(out_channels),
                nn.InstanceNorm3d(out_channels),
                nn.LeakyReLU()
            )
...
class DownBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dropout_p, flag_3d):
        ...
        if not flag_3d:
            ...
            self.maxpool_conv = nn.Sequential(
                nn.MaxPool3d(2),
                ConvBlock(in_channels, out_channels, dropout_p, flag_3d)
            )
...
class UpBlock(nn.Module):
    ...
    def __init__(self, in_channels1, in_channels2, out_channels, dropout_p,
        ...
        if not flag_3d:
            ...
            self.conv = ConvBlock(in_channels2 * 2, out_channels, dropout_p, flag_3d)
...
class Encoder(nn.Module):
    def __init__(self, params):
        ...
        self.flag_3d = self.params['flag_3d']
        assert (len(self.ft_chns) == 5)
        self.in_conv = ConvBlock(
            self.in_chns, self.ft_chns[0], self.dropout[0], self.flag_3d)
        self.down1 = DownBlock(
            self.ft_chns[0], self.ft_chns[1], self.dropout[1], self.flag_3d)
        self.down2 = DownBlock(
            self.ft_chns[1], self.ft_chns[2], self.dropout[2], self.flag_3d)
        self.down3 = DownBlock(
            self.ft_chns[2], self.ft_chns[3], self.dropout[3], self.flag_3d)
        self.down4 = DownBlock(
            self.ft_chns[3], self.ft_chns[4], self.dropout[4], self.flag_3d)
...
class Encoder_DualAtt(nn.Module):
    def __init__(self, params):
        super(Encoder_DualAtt, self).__init__()
        self.params = params
        self.in_chns = self.params['in_chns']
        self.ft_chns = self.params['feature_chns']
        self.n_class = self.params['class_num']
...
class Decoder(nn.Module):
    def __init__(self, params):
        ...
        self.up1 = UpBlock(
            self.ft_chns[4], self.ft_chns[3], self.ft_chns[3], dropout_p=0.0, flag_3d=self.flag_3d)
        self.up2 = UpBlock(
            self.ft_chns[3], self.ft_chns[2], self.ft_chns[2], dropout_p=0.0, flag_3d=self.flag_3d)
        self.up3 = UpBlock(
            self.ft_chns[2], self.ft_chns[1], self.ft_chns[1], dropout_p=0.0, flag_3d=self.flag_3d)
        self.up4 = UpBlock(
            self.ft_chns[1], self.ft_chns[0], self.ft_chns[0], dropout_p=0.0, flag_3d=self.flag_3d)
        if not self.flag_3d:
...
class UNet(nn.Module):
    def __init__(self, in_chns, class_num, flag_3d=False):
        super(UNet, self).__init__()

        params = {'in_chns': in_chns,
                  'feature_chns': [16, 32, 64, 128, 256],
                  'dropout': [0.00, 0.0, 0.0, 0.0, 0.0],
                  'class_num': class_num,
                  'bilinear': True,
                  'acti_func': 'relu',
                  'flag_3d': flag_3d}
...
class UNet_v2(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        self.in_conv1 = nn.Conv2d(in_chns, 64, kernel_size=3, stride=1, padding=1)
        ...
        self.out_conv = nn.Conv2d(64, class_num, kernel_size=1, stride=1, padding=0)
        ...
        self.bn256_4 = nn.BatchNorm2d(256)
    ...
    def forward(self, input):
        c1 = self.relu(self.bn64_1(self.in_conv1(input)))
        c2 = self.relu(self.bn64_2(self.in_conv2(c1)))

        e1_ = self.relu(self.bn128_1(self.conv1_1(self.pooling1(c2))))
        e1 = self.relu(self.bn128_2(self.conv1_2(e1_)))

        e2_ = self.relu(self.bn256_1(self.conv2_1(self.pooling1(e1))))
        e2 = self.relu(self.bn256_2(self.conv2_2(e2_)))

        e3_ = self.relu(self.bn512_1(self.conv3_1(self.pooling1(e2))))
        e3 = self.relu(self.bn512_2(self.conv3_2(e3_)))

        e4_ = self.relu(self.bn512_3(self.conv4_1(self.pooling1(e3))))
        e4 = self.relu(self.bn512_4(self.conv4_2(e4_)))

        d1_1 = self.upsampling1(e4)
        d1_2 = torch.cat([d1_1, e3], dim=1)
        d1_ = self.relu(self.bn256_3(self.conv5_1(d1_2)))
        d1 = self.relu(self.bn256_4(self.conv5_2(d1_)))

        d2_1 = self.upsampling1(d1)
        d2_2 = torch.cat([d2_1, e2], dim=1)
        d2_ = self.relu(self.bn128_3(self.conv6_1(d2_2)))
        d2 = self.relu(self.bn128_4(self.conv6_2(d2_)))

        d3_1 = self.upsampling1(d2)
        d3_2 = torch.cat([d3_1, e1], dim=1)
        d3_ = self.relu(self.bn64_3(self.conv7_1(d3_2)))
        d3 = self.relu(self.bn64_4(self.conv7_2(d3_)))

        d4_1 = self.upsampling1(d3)
        d4_2 = torch.cat([d4_1, c2], dim=1)
        d4_ = self.relu(self.bn64_5(self.conv8_1(d4_2)))
        d4 = self.relu(self.bn64_6(self.conv8_2(d4_)))
