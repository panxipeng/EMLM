...
class BaseDataSets(Dataset):
    def __init__(
        ...
        # else:
            ...
            self.sample_list = os.listdir(self._base_dir + "/test_image_slice")
            self.sample_list = [case[:-3] for case in self.sample_list]
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            ...
            h5f = h5py.File(self._base_dir + "/test_image_slice/{}.h5".format(case), "r")
...
class newBaseDataSets(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ori_image = h5f["image"][:]
        ori_image = np.array(ori_image, dtype=np.float64)
        ori_label = h5f["label"][:]
        ori_label = np.array(ori_label, dtype=np.uint8)
        # lung_area = h5f["lung_area"][:]
        lung = h5f["lung"][:]
        lung = np.array(lung, dtype=np.uint8)
        # if self.split == "train":
        #     lung_area = h5f["lung_area"]
        sample = {"image": ori_image, "label": ori_label, "lung": lung}
        # precessor.Transform_array_to_png(sample["image"], sample["label"], sample["label"], "ori", './')
        # precessor.Transform_tensor_to_png(sample["ori_image"], sample["ori_label"], "test2", './')
        if self.transform is not None:
            for transform in self.transform:
                sample = transform(sample)
                # flag = transform.getName() == "Image_augmentation"
                if transform.getName() == "Image_augmentation":
                    ori_image = copy.deepcopy(sample["image"])
                    ori_label = copy.deepcopy(sample["label"])

            # sample = self.transform(sample)
        else:
            image = torch.from_numpy(ori_image).unsqueeze(0)
            label = torch.from_numpy(ori_label).unsqueeze(0)
            # image = torch.from_numpy(image.astype(np.float32))
            # label = torch.from_numpy(label.astype(np.float32))
            # sample = {"image": image, "label": label, "ori_label": ori_label, "ori_image": ori_image}
        ...
        # sample["label"] = sample["label"] / 1.0
        # precessor = Preprocesser()
        # precessor.Transform_tensor_to_png(sample["image"], sample["label"], "test", './')
        # precessor.Transform_tensor_to_png(sample["ori_image"], sample["ori_label"], "test2", './')
        return sample


class newLesion_and_image_augmentation:
    def __init__(self, lesion_path, labeled_prop, augmentation_proportion=None, init_lesion_count=None, argrate=None,
                 ft_flag=False, racalss_flag=True):
        self.augmentation_proportion = augmentation_proportion
        self.labeled_prop = labeled_prop
        self.lesion_count = init_lesion_count
        self.lesion_path = lesion_path
        self.aug_rate = argrate
        ...
        self.raclass_flag = racalss_flag
    ...
    def __call__(self, sample):
        ...
        if self.ft_flag or (not self.raclass_flag):
            label[label > 0] = 1

        image = torch.from_numpy(image.copy()).unsqueeze(0)
        label = torch.from_numpy(label.copy()).unsqueeze(0)
        # image = image.to(torch.FloatTensor)
        # label = label.to(torch.FloatTensor)
    ...
    def getName(self):
        return "newLesion_and_image_augmentation"
    ...
    def LesionAssemble(self, sample):
        ...
        if not self.raclass_flag:
            ChoicedClassList = ['init'] * self.lesion_count
        else:
        ...
        for CurrentClass in ChoicedClassList:
            ...
            LesionList.sort()
            LesionList = LesionList[:int(len(LesionList) * self.labeled_prop)]
            ...
            ########################
            # 随机选择一层
            random_index = np.random.randint(low=0, high=lesion_arr.shape[0])
    ...
    def Lesion_rot(self, image, label):
        ...
        image = rotate(image, rotation_angle_z, axes=(1, 2), reshape=False, order=0)

        label = rotate(label, rotation_angle_z, axes=(1, 2), reshape=False, order=0)
    ...
    def Lesion_resize(self, image, label):
        ...
        new_image = image
        new_label = label
        new_image = zoom(new_image, resize_factor, order=0)
    ...
    def UpdateLesionCount(self, lesion_count):
...
class Lesion_and_image_augmentation:
    def __init__(self, lesion_path, labeled_prop, augmentation_proportion=None, init_lesion_count=None, argrate=None,
                 ft_flag=False, racalss_flag=True, aug_switch=True, enhencement_strength="Strong"):
        self.enhencement_strength = enhencement_strength
        self.augmentation_proportion = augmentation_proportion
        self.labeled_prop = labeled_prop
        self.lesion_count = init_lesion_count
        self.lesion_path = lesion_path
        self.aug_rate = argrate
        self.aug_switch = aug_switch
        self.LesionClassList = os.listdir(self.lesion_path)
        self.LesionClassList.remove("init")
        self.CC = transforms.CenterCrop(512)
        self.trs = transforms.Normalize(mean=[0.5], std=[0.5])
    ...
    def __call__(self, sample):
        ...
        #     print()
        if np.random.random() < self.aug_rate and self.aug_switch:
            new_sample = self.LesionAssemble(sample)
        else:
            new_sample = sample
        image = new_sample["image"]
        ...
        # if np.max(image>1):
        #     print("Lesion_aug", np.max(image))
        # image[image > 1] = 1
        # image[image < 0] = 0
        if self.enhencement_strength == 'Strong':
        ...
        elif self.enhencement_strength == 'Weak':
        ...
        new_sample = {'image': image, 'label': label}
        return new_sample
    def Switch_aug(self, aug_switch, enhencement_strength):
        self.aug_switch = aug_switch
    ...
    def LesionAssemble(self, sample):
        ...
        for CurrentClass in ChoicedClassList:
            ...
            LesionList = LesionList[:int(len(LesionList) * self.labeled_prop)]
            #######################
            # '''muti-lesions generate one new lesion'''
            # lesion_dirs = [random.sample(LesionList, 1)[0] for i in range(np.random.randint(1, 4))]
            # h5f_list = [h5py.File(os.path.join(lesion_path, lesion), 'r') for lesion in lesion_dirs]
            # lesion_arr_list = [h5f["lesion"][:] for h5f in h5f_list]
            # lesion_mask_list = [h5f["lesion_mask"][:] for h5f in h5f_list]
            # random_idx = np.random.randint(0, len(lesion_dirs))
            # # ori_base = copy.deepcopy(lesion_arr_list[random_idx])
            # new_lesion_arr_base = lesion_arr_list[random_idx]
            # new_lesion_mask_base = lesion_mask_list[random_idx]
            # # for lesion in lesion_arr_list:
            # #     if np.max(lesion>1):
            # #         print("Lesion_aug 1", np.max(lesion))
            # idx = -1
            # for lesion, lesion_mask in zip(lesion_arr_list, lesion_mask_list):
            #     idx += 1
            #     if idx == random_idx:
            #         continue
            #
            ...
            #     # if np.max(lesion) > 1:
            #     #     print(np.max(lesion))
            #     # zoom_ = [lesion.shape[0] / new_lesion_arr_base.shape[0], lesion.shape[1] / new_lesion_arr_base.shape[1],
            #     #          lesion.shape[2] / new_lesion_arr_base.shape[2]]
            #     zoom_ = [new_lesion_arr_base.shape[0] / lesion.shape[0], new_lesion_arr_base.shape[1] / lesion.shape[1],
            #              new_lesion_arr_base.shape[2] / lesion.shape[2]]
            #     new_lesion_arr = ndimage.zoom(lesion, zoom_, order=0)
            #     new_lesion_mask = ndimage.zoom(lesion_mask, zoom_, order=0)
            #
            #     # new_lesion_arr_base += new_lesion_arr
            #     rand_cof = np.random.dirichlet(np.ones(2))
            #     new_lesion_arr_base = np.where((new_lesion_mask > 0) * (new_lesion_mask_base > 0),
            #                                    rand_cof[0] * new_lesion_arr_base + rand_cof[1] * new_lesion_arr,
            #                                    np.where(new_lesion_mask_base > 0, new_lesion_arr_base,
            #                                             np.where(new_lesion_mask > 0, new_lesion_arr, new_lesion_mask_base)))
            #     new_lesion_mask_base = np.where(new_lesion_mask > 0, new_lesion_mask, new_lesion_mask_base)
            # lesion_arr, lesion_mask = new_lesion_arr_base, new_lesion_mask_base
            #######################
            #######################
            lesion = random.sample(LesionList, 1)[0]
    ...
    def Lesion_rot(self, image, label):
        ...
        # image = rotate(image, rotation_angle_x, axes=(0, 1), reshape=False, order=3)
        # image = rotate(image, rotation_angle_y, axes=(0, 2), reshape=False, order=3)
        image = rotate(image, rotation_angle_z, axes=(1, 2), reshape=False, order=0)

        # label = rotate(label, rotation_angle_x, axes=(0, 1), reshape=False, order=0)
        # label = rotate(label, rotation_angle_y, axes=(0, 2), reshape=False, order=0)
        label = rotate(label, rotation_angle_z, axes=(1, 2), reshape=False, order=0)
    ...
    def random_rotate(self, img, label):
        if np.random.uniform() <= self.config['p_rotate']:
            ...
            img = rotate(img, angle, axes=(0, 1), reshape=True, order=0)
...
class Image_augmentation():
    ...
    def __call__(self, sample):
        new_sample = sample
        new_sample["label"][new_sample["label"] > 0] = 1

        # image = np.asarray(image, dtype=np.fl)

        new_sample = self.add_gaussian_noise(new_sample)
        new_sample = self.add_gaussian_blur(new_sample)
        new_sample = self.adjust_brightness(new_sample)
        new_sample = self.adjust_contrast(new_sample)
        # image = self.augment_gamma(image)
        # image = self.totensor(image.copy())
        # label = self.totensor(label.copy())
        new_sample = self.random_rotate(new_sample)
        new_sample= self.random_flip(new_sample)
        new_sample = self.Remove_Small_connected_domain(new_sample)
        # label = self.Remove_Small_connected_domain(label)
        new_sample["image"] = torch.from_numpy(new_sample["image"].copy()).unsqueeze(0)
        new_sample["label"] = torch.from_numpy(new_sample["label"].copy()).unsqueeze(0)
        new_sample["lung"] = torch.from_numpy(new_sample["lung"].copy()).unsqueeze(0)
        # image = self.trs(image)
        new_sample["image"] = self.CC(new_sample["image"])
        new_sample["label"] = self.CC(new_sample["label"])
        new_sample["lung"] = self.CC(new_sample["lung"])

        # new_sample["image"] = new_sample["image"].to(torch.FloatTensor)
        # new_sample["label"] = new_sample["label"].to(torch.FloatTensor)
        # new_sample["lung"] = new_sample["lung"].to(torch.FloatTensor)
        # image = self.trs(image)
        # new_sample = {'image': image, 'label': label, 'lung':lung}
        return new_sample
    ...
    def add_gaussian_noise(self, data):
        ...
        if np.random.uniform() <= self.config['p_gaussian_noise']:
            variance = np.random.uniform(0, self.config['gaussian_noise_variance'])
            noise = np.random.normal(0, variance, size=data['image'].shape)
    ...
    def add_gaussian_blur(self, data):
        ...
        if np.random.uniform() <= self.config['p_gaussian_blur']:
            lower, upper = self.config['gaussian_blur_sigma_range'][0], self.config['gaussian_blur_sigma_range'][1]
            sigma = np.random.uniform(lower, upper)
            data['image'] = gaussian_filter(data['image'], sigma=sigma, order=0)
        return data
    ...
    def adjust_contrast(self, data):
        ...
        if np.random.uniform() <= self.config['p_contras']:
            factor = np.random.uniform(1 - self.config['max_contrast'], 1 + self.config['max_contrast'])
            mn, minm, maxm = data['image'].mean(), data['image'].min(), data['image'].max()

            data['image'] = (data['image'] - mn) * factor + mn
            data['image'] = np.clip(data['image'], a_min=minm, a_max=maxm)
        return data
    ...
    def augment_gamma(self, data, epsilon=1e-7):
        if np.random.uniform() <= self.config['p_gamma']:
            ...
            data['image'] = - data_sample
        return data

    def Remove_Small_connected_domain(self, data):
        image = data['image']
        min_area_thr = 40
        # img1, contours, hier = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)  # contours返回的是轮廓像素点列表，一张图像有几个目标区域就有几个列表值
        num_labels, labels, stats, centroids = cv2.connectedComponentsWithStats(np.array(image, dtype=np.uint8), connectivity=8)
    ...
    def random_rotate(self, data):
        if np.random.uniform() <= self.config['p_rotate']:
            ...
            for key, value in data.items():
                data[key] = rotate(data[key], angle, axes=(0, 1), reshape=True, order=3)
    ...
    def random_flip(self, data):
        ...
        flip_axis = random.sample(flip_range, 1)
        for key, value in data.items():
            data[key] = np.flip(data[key], axis=flip_axis)
